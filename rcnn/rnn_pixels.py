...
def delete_bad_tiles(l8_data, lc_label, canopy_data, pixels, tile_size, max_size=None):
    buffer = math.floor(tile_size / 2)
    cloud_list = [72, 80, 96, 130, 132, 136, 160, 224]
    new_pixels = []
    l8_proj = Proj(l8_data['028012'][0].crs)
    lc_proj = Proj(lc_label.crs)
    canopy_proj = Proj(canopy_data.crs)
    for pixel in pixels:
        r, c = pixel[0]
        ...
        canopy_row, canopy_col = canopy_data.index(canopy_x,canopy_y)
        canopy_data = self.canopy_label.read(1, window=Window(canopy_col-buffer, canopy_row-buffer, tile_size, tile_size))
        flag = True
        if 0 in lc_data or np.nan in lc_data or np.nan in canopy_data or 255 in canopy_data or canopy_data.shape != (self.tile_length, tile_length) or len(np.unique(lc_data)) == 1:
            flag = False
